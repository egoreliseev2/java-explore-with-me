...
public interface EndpointHitRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query(value = "SELECT new ru.practicum.ewm.model.ViewStat(e.uri, e.app, COUNT(DISTINCT e.ip)) " +
            "FROM EndpointHit e " +
    ...
    @Query("SELECT new ru.practicum.ewm.model.ViewStat(e.uri, e.app, COUNT(e.ip)) " +
            "FROM EndpointHit e " +
            "WHERE e.timestamp BETWEEN :start AND :end " +
            "GROUP BY e.app, e.uri " +
            "ORDER BY COUNT(e.ip) DESC")
